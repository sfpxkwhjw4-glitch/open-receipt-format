"use strict";
// Structural integrity test for orf-v0.4.schema.json.
//
...
const SCHEMA_PATH = path.join(__dirname, "orf-v0.4.schema.json");
...
test("decision.orf_version allows 0.1, 0.2, 0.3, and 0.4 (backward compat)", () => {
  ...
  assert.ok(en.includes("0.4"), "decision must allow orf_version 0.4");
});
...
test("outcome.status enum includes held, falsified, undetermined, and partial (v0.4)", () => {
  ...
  assert.ok(en.includes("partial"), "outcome.status must include partial (new in v0.4)");
...
test("aggregate.partial is optional non-negative integer (new in v0.4)", () => {
...
test("sub_outcome.status enum includes partial (new in v0.4)", () => {
  ...
  assert.ok(en.includes("partial"), "sub_outcome.status must include partial (new in v0.4)");
  assert.deepEqual([...en].sort(), ["falsified", "held", "partial", "undetermined"]);
});
...
test("reconcile.orf_version allows 0.2, 0.3, and 0.4", () => {
  ...
  assert.ok(en.includes("0.4"), "reconcile must allow orf_version 0.4");
});
...
test("delegation.orf_version allows 0.3 and 0.4 (delegation is v0.3+)", () => {
  const { orf_version } = schema.definitions.delegation.properties;
  assert.ok(!orf_version.const, "delegation.orf_version must not use const in v0.4 schema");
  assert.ok(orf_version.enum, "delegation.orf_version must use enum in v0.4 schema");
  assert.ok(orf_version.enum.includes("0.3"), "delegation must allow orf_version 0.3");
  assert.ok(orf_version.enum.includes("0.4"), "delegation must allow orf_version 0.4");
});
...
// ─── v0.4-specific: backward compatibility with v0.3 records ────────────────

test("v0.3 decision records (orf_version 0.3) remain valid in v0.4 schema", () => {
  const en = schema.definitions.decision.properties.orf_version.enum;
  assert.ok(en.includes("0.3"), "v0.3 decisions must still be accepted");
});

test("v0.3 outcome records (status: held/falsified/undetermined) remain valid in v0.4 schema", () => {
  const en = schema.definitions.outcome.properties.status.enum;
  for (const s of ["held", "falsified", "undetermined"]) {
    assert.ok(en.includes(s), `v0.3 status '${s}' must remain valid in v0.4 schema`);
  }
});

test("v0.3 delegation records (orf_version 0.3) remain valid in v0.4 schema", () => {
